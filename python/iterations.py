...
data = np.loadtxt("../outputs/Flow Over a cylinder_Re=25.txt", delimiter=",")
...
plt.plot(t[:]*50, np.log10(data[:, 0]), label=r"$u$")
plt.plot(t[:]*50, np.log10(data[:, 1]), label=r"$v$")
plt.plot(t[:]*50, np.log10(data[:, 2]), label=r"$p$")
